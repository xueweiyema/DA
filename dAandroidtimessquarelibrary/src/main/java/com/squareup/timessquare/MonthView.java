...
public class MonthView extends LinearLayout {
    ...
    public static MonthView create(ViewGroup parent, LayoutInflater inflater, DateFormat weekdayNameFormat, Listener listener, Calendar today, int dividerColor,
                                   int dayBackgroundResId, int dayTextColorResId, int titleTextColor, boolean displayHeader, int headerTextColor) {
        ...
        for (int offset = 0; offset < 7; offset++) {
            ...
            // 设置日期抬头(Sun,Mon,Tue,Wed,Thu,Fri,Sat);
            textView.setText(weekdayNameFormat.format(today.getTime()));
            // Log.i(TAG, weekdayNameFormat.format(today.getTime()));
            // Log.i(TAG + " originalDayOfWeek==", originalDayOfWeek + "");
        }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (i < numRows) {
                ...
                for (int c = 0; c < week.size(); c++) {
                    ...
                    // 打印cellDate的值
                    // Log.i(TAG + " cellDate ==", cellDate);
                    // Log.i(TAG + " cell ==", dauser.getDuty(cell.getDate()));
                    if (!cellView.getText().equals(cellDate)) {
                        cellView.setText(cellDate + "\n" + dauser.getDuty(cell.getDate()));
                    }
