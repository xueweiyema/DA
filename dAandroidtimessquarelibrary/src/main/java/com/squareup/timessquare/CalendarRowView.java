...
public class CalendarRowView extends ViewGroup implements View.OnClickListener {
	...
	public void setCellTextColor(int resId) {
		Log.i("setCellTextColor.resId.getChildCount=", getChildCount() + "");
		for (int i = 0; i < getChildCount(); i++) {
			//修改周末文字颜色
			if (i == 0 || i == 6) {
				((TextView) getChildAt(i)).setTextColor(getResources().getColor(R.color.lightpink));
			} else {
				((TextView) getChildAt(i)).setTextColor(getResources().getColor(R.color.orange));
			}
		}
	}

	public void setCellTextColor(ColorStateList colors) {
		Log.i("setCellTextColor.ColorStateList.getChildCount=", getChildCount() + "");
		for (int i = 0; i < getChildCount(); i++) {
