...
public class UpdateService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		Log.i("UpdateService.java", "begin update " + intent.getStringExtra("url"));
		...
		new Thread() {
			public void run() {
				try {
					...
					if (is != null) {
						File rootFile = new File(Environment.getExternalStorageDirectory(), "/DA");
						if (!rootFile.exists() && !rootFile.isDirectory())
							rootFile.mkdir();

						tempFile = new File(Environment.getExternalStorageDirectory(),

						"/DA/" + url.substring(url.lastIndexOf("/") + 1));
						if (tempFile.exists())
							tempFile.delete();
